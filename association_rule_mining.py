...
for index, row in df.iterrows():
    ...
    labels={"Bread":0,"Wine":0,"Eggs":0,"Meat":0,"Cheese":0,"Pencil":0,"Diaper":0,"Bagel":0,"Milk":0}
    row=row.dropna()
    for n in row:
        labels[n]=1
    print(labels)
...
class printFrozen(frozenset):
    def __repr__(self):
        return '([{}])'.format(', '.join(map(repr, self)))

for index, row in rules.iterrows():
   # print()
    print(printFrozen(row["antecedents"]),"->",printFrozen(row["consequents"]))
...
#-->add your python code below
    ...
    prior = row['support'] * len(ohe_df) / len(encoded_vals)
    print("Prior: ", prior)
